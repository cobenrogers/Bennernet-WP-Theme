 ...
 * Wizard
 *
 * @package Ovation_Blog_Whizzie
 * @author Catapult Themes
 * @since 1.0.0
 */

class Ovation_Blog_Whizzie {
	
	protected $version = '1.1.0';
	
	/** @var string Current theme name, used as namespace in actions. */
	protected $ovation_blog_theme_name = '';
	protected $ovation_blog_theme_title = '';
	
	/** @var string Wizard page slug and title. */
	protected $ovation_blog_page_slug = '';
	protected $ovation_blog_page_title = '';
	 ...
	 * @var string
	 */
	protected $ovation_blog_plugin_url = '';

	public $ovation_blog_plugin_path;
	public $parent_slug;
	...
	public function set_vars( $config ) {
		...
		if( isset( $config['ovation_blog_page_slug'] ) ) {
			$this->ovation_blog_page_slug = esc_attr( $config['ovation_blog_page_slug'] );
		}
		if( isset( $config['ovation_blog_page_title'] ) ) {
			$this->ovation_blog_page_title = esc_attr( $config['ovation_blog_page_title'] );
		}
		if( isset( $config['steps'] ) ) {
			$this->config_steps = $config['steps'];
		}
		
		$this->ovation_blog_plugin_path = trailingslashit( dirname( __FILE__ ) );
		$relative_url = str_replace( get_template_directory(), '', $this->ovation_blog_plugin_path );
		$this->ovation_blog_plugin_url = trailingslashit( get_template_directory_uri() . $relative_url );

		$ovation_blog_current_theme = wp_get_theme();

		$this->ovation_blog_theme_title = $ovation_blog_current_theme->get( 'Name' );
		$this->ovation_blog_theme_name = strtolower( preg_replace( '#[^a-zA-Z]#', '', $ovation_blog_current_theme->get( 'Name' ) ) );
		
		$this->ovation_blog_page_slug = apply_filters( $this->ovation_blog_theme_name . '_theme_setup_wizard_ovation_blog_page_slug', $this->ovation_blog_theme_name . '-wizard' );
		$this->parent_slug = apply_filters( $this->ovation_blog_theme_name . '_theme_setup_wizard_parent_slug', '' );
	...
	public function init() {
		...
		add_action( 'wp_ajax_ovation_blog_setup_widgets', array( $this, 'ovation_blog_setup_widgets' ) );
		
	}
	
	public function enqueue_scripts() {
		wp_enqueue_style( 'ovation-blog-dashboard-style', get_template_directory_uri() . '/inc/dashboard/assets/css/dashboard.css');
		wp_register_script( 'ovation-blog-dashboard-script', get_template_directory_uri() . '/inc/dashboard/assets/js/dashboard.js', array( 'jquery' ), time() );
		wp_localize_script( 
			'ovation-blog-dashboard-script',
			'whizzie_params',
			array(
				'ajaxurl' 		=> admin_url( 'admin-ajax.php' ),
				'wpnonce' 		=> wp_create_nonce( 'whizzie_nonce' ),
				'verify_text'	=> esc_html( 'verifying', 'ovation-blog' )
			)
		);
		wp_enqueue_script( 'ovation-blog-dashboard-script' );
	}
	...
	public function set_tgmpa_url() {
		...
		$this->tgmpa_menu_slug = apply_filters( $this->ovation_blog_theme_name . '_theme_setup_wizard_tgmpa_menu_slug', $this->tgmpa_menu_slug );
		$tgmpa_parent_slug = ( property_exists( $this->tgmpa_instance, 'parent_slug' ) && $this->tgmpa_instance->parent_slug !== 'themes.php' ) ? 'admin.php' : 'themes.php';
		$this->tgmpa_url = apply_filters( $this->ovation_blog_theme_name . '_theme_setup_wizard_tgmpa_url', $tgmpa_parent_slug . '?page=' . $this->tgmpa_menu_slug );
	}
	...
	public function menu_page() {
		add_theme_page( esc_html( $this->ovation_blog_page_title ), esc_html( $this->ovation_blog_page_title ), 'manage_options', $this->ovation_blog_page_slug, array( $this, 'wizard_page' ) );
	}
	...
	public function wizard_page() { 
		...
		if ( ! class_exists( 'TGM_Plugin_Activation' ) || ! isset( $GLOBALS['tgmpa'] ) ) {
			die( esc_html__( 'Failed to find TGM', 'ovation-blog' ) );
		}
		...
		$ovation_blog_theme_title = $ovation_blog_theme->get( 'Name' );

		?>
		<div class="getting-started__header">
			<div class="header-box-left">
				<h2><?php echo esc_html( $ovation_blog_theme_title ); ?></h2>
		...
		<div class="import-box">
			<div class="whizzie-wrap">
				<div class="demo_content">
					<?php
						$ovation_blog_steps = $this->get_steps();
						echo '<ul class="whizzie-menu">';
						foreach ( $ovation_blog_steps as $ovation_blog_step ) {
							$class = 'step step-' . esc_attr( $ovation_blog_step['id'] );
							echo '<li data-step="' . esc_attr( $ovation_blog_step['id'] ) . '" class="' . esc_attr( $class ) . '">';
							printf( '<h2>%s</h2>', esc_html( $ovation_blog_step['title'] ) );

							$content = call_user_func( array( $this, $ovation_blog_step['view'] ) );
							...
							if ( isset( $ovation_blog_step['button_text'] ) && $ovation_blog_step['button_text'] ) {
								printf( 
									'<div class="button-wrap"><a href="#" class="button button-primary do-it" data-callback="%s" data-step="%s">%s</a></div>',
									esc_attr( $ovation_blog_step['callback'] ),
									esc_attr( $ovation_blog_step['id'] ),
									esc_html( $ovation_blog_step['button_text'] )
								);
							}
							echo '</li>';
						}
					...
					<ul class="whizzie-nav">
						...
						$step_number = 1;	
						foreach ( $ovation_blog_steps as $ovation_blog_step ) {
							echo '<li class="nav-step-' . esc_attr( $ovation_blog_step['id'] ) . '">';
			...
			<div class="box-container">
				<div class="box-left-main">
					<div class="leftbox">
						...
						<p><?php esc_html_e('To setup the ovation blog theme follow the below steps.','ovation-blog'); ?></p>
						...
						<p><?php esc_html_e('Go to dashboard >> Appearance >> Customize >> Custom Page Settings >> Social Media >> Add social links.','ovation-blog'); ?></p>
						...
						<p><?php esc_html_e('Go to dashboard >> Appearance >> Customize >> Custom Page Settings >> Footer Text >> Add copyright text.','ovation-blog'); ?></p>
						...
						<p><?php esc_html_e('To setup the home page follow the below steps.','ovation-blog'); ?></p>
						...
						<a class="dashboard_add_new_page button-primary" href="<?php echo esc_url(admin_url( 'post-new.php?post_type=page' ) ); ?>" target="_blank"><?php esc_html_e('Add New Page','ovation-blog'); ?></a>
						...
						<p><?php esc_html_e('Go to dashboard >> Appearance >> Custom Page Settings >> Customize >> Slider Settings >> Select post.','ovation-blog'); ?></p>
						...
						<p><?php esc_html_e('Go to dashboard >> Appearance >> Customize >> Custom Page Settings >> Popular Post >> Select post category.','ovation-blog'); ?></p>
						...
						<h4><?php esc_html_e('5. Setup Top Categories','ovation-blog'); ?></h4>
						<p><?php esc_html_e('Go to dashboard >> Post >> Add New Post Category >> Add New Post >> Add title, content, select post category and featured image >> Publish it.','ovation-blog'); ?></p>
						<p><?php esc_html_e('Go to dashboard >> Appearance >> Customize >> Custom Page Settings >> Top Categories >> Add headings and select post category.','ovation-blog'); ?></p>
				...
				<div class="box-right-main">
					...
					<div class="pro-links">
						<div class="pro-links-inner">
							...
							<a class="button-primary bundle-btn" href="<?php echo esc_url( OVATION_BLOG_BUNDLE_LINK ); ?>" target="_blank"><?php esc_html_e('WordPress Theme Bundle (120+ Themes at Just $89)', 'ovation-blog'); ?></a>
					</div>
	...
	public function get_steps() {
		$ovation_blog_dev_steps = $this->config_steps;
		$ovation_blog_steps = array( 
			'plugins' => array(
				'id'			=> 'plugins',
			...
			'widgets' => array(
				...
				'callback'		=> 'ovation_blog_install_widgets',
		...
		if( $ovation_blog_dev_steps ) {
			// Configurable elements - these are the only ones the dev can update from dashboard-settings.php
			$can_config = array( 'title', 'icon', 'button_text', 'can_skip' );
			foreach( $ovation_blog_dev_steps as $ovation_blog_dev_step ) {
				// We can only proceed if an ID exists and matches one of our IDs
				if( isset( $ovation_blog_dev_step['id'] ) ) {
					$id = $ovation_blog_dev_step['id'];
					if( isset( $ovation_blog_steps[$id] ) ) {
						foreach( $can_config as $element ) {
							if( isset( $ovation_blog_dev_step[$element] ) ) {
								$ovation_blog_steps[$id][$element] = $ovation_blog_dev_step[$element];
							}
						}
					}
				}
		...
		<p class="note">
		    ...
		    echo esc_html(
		        sprintf(
		            ...
		            __( 'If your website is already live and containing data, please make a backup. This importer will override the %s\'s new customizable values.', 'ovation-blog' ),
		            $this->ovation_blog_theme_title
		        )
		    );
		    ?>
		</p>
	...
	public function setup_plugins() {
		if ( ! check_ajax_referer( 'whizzie_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found','ovation-blog' ) ) );
		}
		$json = array();
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating Plugin','ovation-blog' ),
				);
				break;
			}
		}
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating Plugin','ovation-blog' ),
				);
				break;
			}
		}
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing Plugin','ovation-blog' ),
				);
				break;
			}
		}
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','ovation-blog' ) ) );
		}
		exit;
	}
	 ...
	 * @since 1.1.0
	 */
	public function ovation_blog_setup_widgets(){
