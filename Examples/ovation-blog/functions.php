...
function ovation_blog_setup() {
	...
	if ( ! defined( 'OVATION_BLOG_BUNDLE_LINK' ) ) {
	define('OVATION_BLOG_BUNDLE_LINK',__('https://www.ovationthemes.com/products/wordpress-bundle','ovation-blog'));
	}
	if ( ! defined( 'OVATION_BLOG_BUNDLE_LINK' ) ) {
	define('OVATION_BLOG_BUNDLE_LINK',__('https://www.ovationthemes.com/products/wordpress-bundle','ovation-blog'));
	}
...
function ovation_blog_custom_admin_notice() {
    ...
    if ( ! get_option('dismissed-get_started_notice', FALSE ) )  {
        ...
        $ovation_blog_theme_name = strtolower( preg_replace( '#[^a-zA-Z]#', '', $ovation_blog_theme->get( 'Name' ) ) );
		$ovation_blog_demo_page_slug = apply_filters( $ovation_blog_theme_name . '_theme_setup_wizard_ovation_blog_page_slug', $ovation_blog_theme_name . '-wizard' );
		$ovation_blog_expected_screen_id = 'appearance_page_' . $ovation_blog_demo_page_slug;
        if ( $ovation_blog_current_screen && $ovation_blog_current_screen->id !== $ovation_blog_expected_screen_id ) { ?>
            <div class="notice notice-info is-dismissible" data-notice="get_started_notice">
                <div class="notice-div">
                    ...
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" href="<?php echo esc_url( 'themes.php?page=' . $ovation_blog_demo_page_slug ); ?>"><?php _e( 'Begin Installation', 'ovation-blog' ); ?></a>
                    </div>
                </div>
            </div>
        <?php
...
// Date formatting
function ovation_blog_display_shop_date() {
    // Get the date type option
    $ovation_blog_date_type = get_theme_mod( 'ovation_blog_date_type', 'published' );

    // Determine the date to display based on the type
    if ( $ovation_blog_date_type === 'modified' && get_the_modified_time( 'U' ) !== get_the_time( 'U' ) ) {
        $ovation_blog_date_to_display = get_the_modified_date( get_option( 'date_format' ) );
    } else {
        $ovation_blog_date_to_display = get_the_date( get_option( 'date_format' ) );
    }

    // Output the date HTML

    echo esc_html( $ovation_blog_date_to_display );
}
